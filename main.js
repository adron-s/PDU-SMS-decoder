...
class PduDecoder {
	...
	parse_sca(buf){
		...
		num = this.cut_nlF(this.number_revert(num)).join('');
	...
	vp_decode(buf){
		let res;
		let len = 7;
		buf = this.number_revert(Array.from(buf.slice(0, len)));
	...
	decode_header(buf){
		let len;
		[ len, this.smsc ] = this.parse_sca(buf, 0); //Номер телефона Центра SMS (может не указываться)
		buf = buf.slice(len); //пропустим байты SCA
		let pdu_type = this.pdu_type = buf[0]; //Тип PDU
		...
		//console.dir(but2hex8(buf), { maxArrayLength: 7 });
	...
	decode(buf){
		...
		if(this.dcs == 0){ //кодировка gsm7
			this.body = this.gsm7_to_str(buf);
			...
			this.body = String.fromCharCode.apply(null, buf16);
		}
		if(this.body)
			return true;
		return false;
...
let bufs = [ buf1, buf2, buf3, buf4 ]; */
let bufs = [ ];
bufs = require('fs').readFileSync("./data.txt").toString().split("\n");
let pdu_decoder = new PduDecoder();
bufs.forEach((buf, i) => {
	//console.log(i);
	if(pdu_decoder.decode(buf) === false)
		return;
	console.log(pdu_decoder.toString());
	console.log("");
});
